...
function runSamplingComparison_triang_weighted(θ, θ_weighted, κs, aη, bη, mcoef; discretization, boxsize=100, numberOfSamplingRuns=250, prefix="linearweight", suffix="")
    #If the file exists, we add to the previously run tests. Else, we set everything to 0.
    θkeys = keys(θ_weighted)
    ourmodel = Dict()
    no_other = Dict()
...
function runTest_twoBestCovers(; boxsize=1, numberOfSamplingRuns=100, prefix="linearweight", suffix="4,9", discretization=20)
    @var κ[1:12]
    ...
    #plotAllCovers(hexPoints, triangconfigurations, lineconfigurations)
    runSamplingComparison_weighted([], θ_weighted, κ, aη, bη, mcoef; boxsize=boxsize, numberOfSamplingRuns=numberOfSamplingRuns, prefix=prefix, suffix=suffix, discretization=discretization)    
end
